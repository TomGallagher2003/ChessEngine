...
import static com.example.chess.MoveEngine.getEngineMove;
import static com.example.chess.MoveEngine.getOpeningMove;
import static com.example.chess.MoveGenerator.generateLegalMovesPlayer;
import static com.example.chess.Utility.OpeningParser.getMoveString;
...
public class BoardManager {
    ...
    public BoardManager(GridPane pane, int depth) {
        ...
        openings.parseOpenings(OPENING_FILENAMES);
    }
    ...
    public void makeEngineMove() {
        Move move = null;
        if(moveCount < 8){
            move = getOpeningMove(playedMoves, position);
        }
        if(move == null){
            move = getEngineMove(position, depth);
        }
