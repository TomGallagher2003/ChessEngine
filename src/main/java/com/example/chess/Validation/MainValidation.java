...
public class MainValidation {
    ...
    public static boolean isValidMove(int oldRow, int oldCol, int newRow, int newCol, Position position) {
        ...
        if (position.getPieceValue(newRow, newCol) * piece > 0) {
            return false;
        }
        // Direct to individual validation functions by piece.
        if (piece == BLACK_PAWN) {
            return isValidMoveBlackPawn(oldRow, newRow, oldCol, newCol, position);
        } else if (piece == WHITE_PAWN) {
            return isValidMoveWhitePawn(oldRow, newRow, oldCol, newCol, position);
            ...
            return isValidMoveRook(oldRow, newRow, oldCol, newCol, position);
        } else if (piece == BLACK_BISHOP || piece == WHITE_BISHOP) {
            return isValidMoveBishop(oldRow, newRow, oldCol, newCol, position);
        } else if (piece == BLACK_QUEEN || piece == WHITE_QUEEN) {
            return isValidMoveQueen(oldRow, newRow, oldCol, newCol, position);
        } else if (piece == BLACK_KING) {
            return isValidMoveBlackKing(oldRow, newRow, oldCol, newCol, position);
        } else if (piece == WHITE_KING) {
            return isValidMoveWhiteKing(oldRow, newRow, oldCol, newCol, position);
        }
        return false;
    }
}
